...
$(document).ready(function() {
	function handleMessageFromBackground(msg) {
  	...
  	$('#update').click(function(evt) {
  		...
  		console.log(mixer);
  		backgroundPageConnection.postMessage({ type: 'mixer_update', data: mixer });
  	});
  	...
  	function updateNodes(nodes) {
  		...
  		for (var i = 0; i < nodes.sources.length; i++) {
  			...
  			let input = $('<input type="radio" name="sources" value="' + node.id + '" id="source_' + i + '" />');
  			if (node.active) {
  				input.attr('checked', 'checked');
  			}
  			let label = $('<label for="source_' + i + '">' + node.title + '</label>');
  			let li = $('<li>');
  			li.append(input);
  			li.append(label);
  			sourcesUL.append(li);
  		...
  		for (var i = 0; i < nodes.destinations.length; i++) {
  			...
  			let input = $('<input type="radio" name="destinations" value="' + node.id + '" id="destination_' + i + '" />');
  			if (node.active) {
  				input.attr('checked', 'checked');
  			}
  			let label = $('<label for="destination_' + i + '">' + node.title + '</label>');
  			let li = $('<li>');
  			li.append(input);
  			li.append(label);
