...
handleNewPopupConnection = function(port) {
  ...
  for (let p of nodes) {
    let node = p[1];
    if (node.isSource) {
      sourceTitles.push({id: node.id, title: node.title, active: node.sourceActive});
    }
    if (node.isDestination) {
      destinationTitles.push({id: node.id, title: node.title, active: node.destinationActive});
    }
  }
  port.postMessage({
    type: 'audio_nodes',
  ...
  for (var [sourceId, source] of mixer.sources.entries()) {
    if (skipId !== sourceId) { 
      try {
       ...
       console.warn('Error source postMessage, probably disconnected', sourceId, ex);
      }
    }
  }
...
function updateMixer(mixerMsg, port) {
  ...
  if (mixer.destination.id != mixerMsg.destination.id) {
    ...
    // 
    let dest = nodes.get(parseInt(mixerMsg.destination.id));
    if (!dest) {
      /// should not happen
    ...
    mixer.destination.port.postMessage({ type: 'audio_prepare_receive', mixer: mixerMsg }); 
  }
  for (var i = mixerMsg.sources.length - 1; i >= 0; i--) {
    // should be only one
    let sourceId = parseInt(mixerMsg.sources[i].id);
