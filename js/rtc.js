...
(function() {
	...
	AudioNode.prototype.connect = wrapNativeFunction(AudioNode.prototype.connect, connectDecorator);
	AudioNode.prototype.disconnect = wrapNativeFunction(AudioNode.prototype.disconnect, disconnectDecorator);
	...
	AudioContext = function() {
		var ctx = createBaseAudioContextSubclass(nativeAudioContextConstructor, Array.prototype.slice.call(arguments), false);
	...
	if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
		navigator.mediaDevices.getUserMedia = wrapNativeFunction(navigator.mediaDevices.getUserMedia, function(nativeGetUserMedia, args) {
			return nativeGetUserMedia.apply(this, args)
	...
	if (navigator.getUserMedia) {
		navigator.getUserMedia = wrapNativeFunction(navigator.getUserMedia, function(nativeGetUserMedia, args) {
			var args_ = Array.prototype.slice.call(args);
			// let _cb = function(stream) {
			// 	args_[1](prepareUserMediaStream(stream));
			// }
			navigator.mediaDevices.getUserMedia(args_[0])
				.then(function(stream) {
					// already decorated
					args_[1](stream);
				})
	...
	AudioContext.prototype.createMediaStreamSource = wrapNativeFunction(AudioContext.prototype.createMediaStreamSource, function(nativeCreate, originalArguments) {
		console.log('createMediaStreamSource', originalArguments[0], userMediaStreams);
	...
	function addRemoteStream(evt) {
		...
		var remoteStreamNode = AudioPipes.WebAudio.audioContext.createMediaStreamSource(evt.stream);
	...
	function answer(msg) {
		...
		AudioPipes.RTC.peerConnection.setRemoteDescription(new RTCSessionDescription(msg.offer))
			.then(function() {
				return AudioPipes.RTC.peerConnection.createAnswer();
			})
			.then(function(sessionDescription) {
				console.log('answer', sessionDescription.sdp);
				return AudioPipes.RTC.peerConnection.setLocalDescription(sessionDescription);				
			})
			.then(function() {
				sendMessage({
					type: 'rtc_answer',
	...
	function connect(answer) {
		AudioPipes.RTC.peerConnection.setRemoteDescription(new RTCSessionDescription(answer))
			.then(function() {
	...
	function makeOffer() {
		...
		attachDataChannel(AudioPipes.RTC.peerConnection.createDataChannel('record_control', {
			ordered: false, // just do it fast, it will be reliable
		}));
		AudioPipes.RTC.peerConnection.createOffer({voiceActivityDetection: false, offerToReceiveAudio: true})
			.then(function(offer) {
	...
	function createBaseAudioContextSubclass(nativeConstructor, argumentsList, isOffline) {
		// Null is the context. We cannot append to Arguments because it's not a
		// list. We convert it to a list by slicing.
		var newContext = (new (Function.prototype.bind.apply(nativeConstructor, [null].concat(argumentsList))));
		return newContext;
	};	
	...
	*/
	...
	function addRecordButton() {
		...
		AudioPipes.WebAudio.recordButton.style.borderColor = 'rgba(255,255,255,0.4)';
